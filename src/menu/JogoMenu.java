...
public class JogoMenu {
    ...
    /*
     * Esse método vai decidir em qual arquivo as operações do nosso CRUD serão
     * feitas.
     * Caso já tenha sido feita uma ordenação ele vai abrir o ordenado, caso
     * contrário, ele abre o outro
     */
    ...
    private static void procurarJogo() {
        ...
        try { // Vai fazer a verificação se o arquivo já foi ordenado para saber em qual
              // arquivo atuar
            ...
            if (idProcurado > 0) { // Verifica o se o ID é válido
                ...
                if (jogo == null) {
                    System.out.println(
                            "Não foi possível encontrar esse jogo. Verifique se ele não foi removido ou se essa posição existe");
                }
    ...
    private static void atualizaJogo() {
        ...
        try {
            ...
            if (idAtualizar > 0) { //  Verifica se o ID é valido.
                ...
                atualizado = CRUD.update(jogo, raf); // Chamamos o método update para atualizar e substituir os valores
                                                     // antigos pelos valores inseridos
        ...
        try {
            ...
            if (idDeletar > 0) {
                ...
                if (deletado) {
                    ...
                    System.out.println(
                            "Não foi possível excluír o jogo. Verifique se a posição é existente ou se ele já não foi removido. ");
                }
            }
            raf.close();
        } catch (IOException e) {
    ...
    public static void ordenarJogos() {
        try {
            ...
            OrdenacaoExterna ordenacao = new OrdenacaoExterna(arquivoOriginal, arquivoOrdenado, numCaminhos,
                    tamanhoBloco);
